    ...
    def put(self, request, format=None):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            if 'profile_pic' in request.data:
                ...
                return Response({"status":True, "message":"Upload Profile Successfully", "data":serializer.data}, status=status.HTTP_200_OK)
    ...
    def delete(self,request,pk):
        ...
        for x in transactions:
            if x.income_from_id is not None:
                ...
                update_amount = int(income_from_id_data[0].amount) + int(x.amount)
    ...
    def delete(self,request,pk):
        ...
        for x in transactions:
            if x.income_to_id is not None:
                ...
                update_amount = int(income_to_id_data[0].amount) - int(x.amount)
...
class TransactionView(APIView):
        ...
        if request.data != {}:
            ...
            if (('amount' in request.data and 'source' in request.data and 'income_to' in request.data) and ('income_from' not in request.data) and ('expense' not in request.data) and ('goal' not in request.data)):
                if (request.data["amount"] != "" and request.data["source"] != ""  and request.data["income_to"] != ""):
                    ...
                    source_amount = int(source.spent_amount) + int(request.data["amount"])
                    income_amount = int(income.amount) + int(request.data["amount"])
                ...
                if (request.data["amount"] != "" and request.data["income_from"] != "" and request.data["income_to"] != ""):
                    ...
                    income_from_amount = int(income_from.amount) - int(request.data["amount"])
                    income_to_amount = int(income_to.amount) + int(request.data["amount"])
                ...
                if (request.data["amount"] != "" and request.data["income_from"] != "" and request.data["goal"] != ""):
                    ...
                    income_amount = int(income.amount) - int(request.data["amount"])
                    goal_amount = int(goal.added_amount) + int(request.data["amount"])
                ...
                if (request.data["amount"] != "" and request.data["income_from"] != "" and request.data["expense"] != ""):
                    ...
                    income_amount = int(income.amount) - int(request.data["amount"])
                    expense_amount = int(expense.spent_amount) + int(request.data["amount"])
    ...
    def put(self, request, pk=None, format=None):
        if pk is not None and pk != "":
            ...
            if (len(source) > 0 and len(income_to) > 0 and len(goal) <= 0 and len(income_from) <= 0 and len(expense) <= 0):
                ...
                if int(request.data["amount"]) > int(transaction.amount):
                    updated_transfer_amount = int(request.data["amount"]) - int(transaction.amount)
                    source_amount = int(source[0].spent_amount) + int(updated_transfer_amount)
                    income_to_amount = int(income_to[0].amount) + int(updated_transfer_amount)
                elif int(request.data["amount"]) < int(transaction.amount):
                    updated_transfer_amount =  int(transaction.amount) - int(request.data["amount"])
                    source_amount = int(source[0].spent_amount) - int(updated_transfer_amount)
                    income_to_amount = int(income_to[0].amount) - int(updated_transfer_amount)
                elif int(request.data["amount"]) == int(transaction.amount):
                ...
                if int(request.data["amount"]) > int(transaction.amount):
                    updated_transfer_amount = int(request.data["amount"]) - int(transaction.amount)
                    income_from_amount = int(income_from[0].amount) - int(updated_transfer_amount)
                    income_to_amount = int(income_to[0].amount) + int(updated_transfer_amount)
                elif int(request.data["amount"]) < int(transaction.amount):
                    updated_transfer_amount =  int(transaction.amount) - int(request.data["amount"])
                    income_from_amount = int(income_from[0].amount) + int(updated_transfer_amount)
                ...
                if int(request.data["amount"]) > int(transaction.amount):
                    updated_transfer_amount = int(request.data["amount"]) - int(transaction.amount)
                    income_from_amount = int(income_from[0].amount) - int(updated_transfer_amount)
                    goal_amount = int(goal[0].added_amount) + int(updated_transfer_amount)
                elif int(request.data["amount"]) < int(transaction.amount):
                    updated_transfer_amount =  int(transaction.amount) - int(request.data["amount"])
                    income_from_amount = int(income_from[0].amount) + int(updated_transfer_amount)
                    goal_amount = int(goal[0].added_amount) - int(updated_transfer_amount)
                ...
                if int(request.data["amount"]) > int(transaction.amount):
                    updated_transfer_amount = int(request.data["amount"]) - int(transaction.amount)
                    income_from_amount = int(income_from[0].amount) - int(updated_transfer_amount)
                    expense_amount = int(expense[0].spent_amount) + int(updated_transfer_amount)
                elif int(request.data["amount"]) < int(transaction.amount):
                    updated_transfer_amount =  int(transaction.amount) - int(request.data["amount"])
                    income_from_amount = int(income_from[0].amount) + int(updated_transfer_amount)
                    expense_amount = int(expense[0].spent_amount) - int(updated_transfer_amount)
    ...
    def get(self, request, pk=None, format=None):
        ...
        if transcation[0].periodic is not None:
            periodic = Periodic.objects.get(id=str(transcation[0].periodic))

        if transcation[0].location is not None:
            location = Location.objects.get(id=str(transcation[0].location))
    ...
    def delete(self, request, pk=None, format=None):
        if pk is not None and pk != '':
            ...
            if len(income_from) > 0 and len(expense) > 0 and len(income_to) <= 0 and len(goal) <= 0 and len(source) <= 0:
                ...
                income_from_amount = int(income_from[0].amount) + int(transaction.amount) 
                expense_amount = int(expense[0].spent_amount) - int(transaction.amount)
                ...
                income_from_amount = int(income_from[0].amount) + int(transaction.amount) 
                income_to_amount = int(income_to[0].amount) - int(transaction.amount)
                ...
                source_amount = int(source[0].spent_amount) - int(transaction.amount) 
                income_to_amount = int(income_to[0].amount) - int(transaction.amount)
                ...
                income_from_amount = int(income_from[0].amount) + int(transaction.amount)
                goal_amount = int(goal[0].added_amount) - int(transaction.amount)
